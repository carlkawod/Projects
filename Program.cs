...
{
    // 1. Read the username and password sent by the frontend
    ...
    // 3. Query the database to see if a matching row exists
    // NOTE: I am assuming your table is named 'Faculty' because of the FacultyID column.
    // If your table is named something else (like 'Users'), change it in the text below!
    var command = connection.CreateCommand();
    command.CommandText = "SELECT FullName FROM Faculty WHERE UserName = @user AND Password = @pass";
    command.Parameters.AddWithValue("@user", request.Username);
    command.Parameters.AddWithValue("@pass", request.Password);

    // ExecuteScalar returns the very first column of the first row it finds (which is FullName)
    var fullName = command.ExecuteScalar() as string;

    // 4. Check the result
    if (fullName != null)
    {
        // A match was found in the database! We let them in.
        return Results.Ok(new { 
            message = "Login successful", 
            token = "fake-jwt-token-123", // You can keep this fake token for now to keep your frontend happy
            name = fullName 
        });
    }

    // No match was found, kick them back to the login screen
...
app.MapPost("/create-course", async (HttpContext http) =>
{
    ...
    var command = connection.CreateCommand();
    command.CommandText = "INSERT INTO Course (CourseID, CourseName, Credits) VALUES (@id, @name, @credits)";
